...
**Открытие (PMID 36693370, 42283151):** Центросома в C. elegans окружена 3D ER-мембранной сетью — центрикулумом. Это опровергает догму «центросома = membraneless organelle».

**Механическая модель для возрастных изменений:**
1. Центрикулум действует как MT-фильтр: блокирует элонгацию микротрубочек
2. Размер центрикулума ↔ размер PCM ↔ MT-нуклеация
3. **Гипотеза CEDAR:** возраст-зависимый ER-стресс → повреждение центрикулума → нарушение MT-фильтрации → ошибки митоза → сенесценс
4. PCM-компактизация (SPD-5 density ↑ при ↓ центрикулума) — возможный компенсаторный механизм
...
> **⚠️ DID-РНК — спекулятивная модель (2026-07-17).** Гипотеза о DID (Differentiation-Inducing Determinants) как РНК, которая через обратную транскрипцию встраивается в геном, не имеет экспериментальных данных. Основная теория CEDAR не зависит от DID-РНК. DID-РНК сохранена как интересная, но неподтверждённая спекуляция в отдельном документе `DID_Centricle_Dve_Linii.md`.
...
1. **Центриоль стареет по второму закону термодинамики — как любая вещественная структура.** Не от делений, не от участия в дифференцировке. Энтропия накапливается пассивно и универсально — у центриолей, белков, липидов, всех молекул.
   ...
   **Энтропия — не храповик и не барьер.** Храповик — активная регуляция (CAMC, NANOG, cilium). Энтропия ЛОМАЕТ активную регуляцию → клетка не возвращается к тотипотентности, а становится раковой.
...
**C. elegans: элиминация происходит, но у соматических клеток нет сброса генных сетей (GRN reset).** Без Reprogram и Rebuild генным сетям нечем переключиться. Только Eliminate → клетка остаётся в текущем состоянии. **Механизм элиминации центриолей в соматических клетках C. elegans остаётся неизвестным** — молекулярные исполнители priming и execution стадий не идентифицированы (Kalbfuss & Gönczy 2023, PMID 37963546). Трёхстадийная модель (maintenance → priming → execution) феноменологическая. Подтверждено Пьером Гёнчи лично (18 июл 2026): «Not that I know» — нет скрытой литературы; «Your guess is as good as mine» — поле открыто.

**Ключевые цифры из Gönczy lab (2023):**
- ~88% клеток теряют центриоли во время эмбриогенеза C. elegans (Kalbfuss & Gönczy 2023 Sci Adv, PMID 37256957)
- Элиминация стереотипна: всегда в одно и то же время в данном типе клеток
- **Cell fate, не cell age, определяет centriole fate** (трансдетерминация меняет судьбу центриоли)
- **Нет diffusible elimination activity** в L1 личинках (Kalbfuss, Berger & Gönczy 2023 Dev Biol, PMID 37414202)
- Ооциты: Polo/PCM-зависимый механизм. Сома: другой механизм
- **ANA1/CEP295** — ключевой стабилизатор центриолярной стенки (Pimenta-Marques 2023, doi:10.1038/s44319-023-00020-6). Рекомендован Гёнчи как отправная точка
...
## 0.4. Cartwheel/SAS-6 Limitation + Direct Centriole-Age Test (Meyer review, 2026-08-13)

> **Замечание D. Meyer (CECAD) — принято, внесено в дизайн:**

**1. Картуэлл/SAS-6 не персистентен.** Картуэлл разбирается при созревании центриоли и в митозе (Huang & Zhang 2025, PMID 39614048) и теряется при цилиогенезе в постмитотических нейронах C. elegans (Li et al. 2017, PMID 28743734; Serwas & Dammermann 2017, PMID 28411189). Поэтому SAS-6 не годится ни как маркер долгоживущей структуры, ни как мишень деградации в зрелых клетках. **Персистентные мишени:** триплет-микротрубочки, внутренний скэффолд (Le Guennec et al. 2020, PMID 32110738, cryo-EM ~4 нм), appendage-кольца. SAS-6 — только там, где картуэлл персистентен (проценриоли; делящиеся клетки C. elegans).

**2. Генетические манипуляции ≠ постепенное повреждение.** Потеря/оверэкспрессия компонентов → дефекты биогенеза, p53, сенесценс — это loss-of-function, а не доказательство накопления повреждений старой центриоли. Дизайн: структурный validation-gate (U-ExM показывает изменение старого скэффолда до интерпретации транскриптома), гипоморфы, острый/хронический тайминг, честная pre-registered интерпретация.

**3. Прямой тест возраста центриоли (первый эксперимент, Фаза 0).** Pulse-label центриолей через асимметричные деления → U-ExM метрики M1–M5 отдельно для старой (материнской) и новой (дочерней) центриоли в одном клеточном фоне. Предсказание: повреждения отслеживают физический возраст отдельной центриоли, а не возраст культуры. Самый дешёвый и прямой тест центральной посылки.

**4. Грант-трек:** VolkswagenStiftung Pioneering Research (дедлайн 27 авг 2026) — centriole-as-counter; lead applicant CECAD (Meyer), GLA — партнёр. Документы: Marketing/DREAM_CEDAR/. Параллельно EIC (28 окт) — разные темы, не конфликтуют.
