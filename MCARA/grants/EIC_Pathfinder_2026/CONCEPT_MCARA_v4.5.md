...
**Дата:** 2026-07-08
**Версия:** v4.6 — Stress Integrator + Hardware/Software Distinction; Meyer-тест (pulse-labeling mother-vs-daughter) в Фазе 0; картуэлл-ограничение (Li 2017, Serwas 2017, Huang 2025); PMID верифицированы (2026-08-13)
...
### CAASM: три конкретных молекулярных reader'а
...
Jeffries et al. (2019, PMID 30596512): **невозможность реабсорбировать первичную цилию достаточна для индукции клеточного сенесценса.** Центриоль контролирует и сборку, и разборку цилии → возраст-зависимые изменения CAASM могут нарушать механизм реабсорбции, запирая клетку в реснитчатом, неделящемся состоянии.
...
|:--:|------|----------|:------:|---------|
| **C1** | Центриолярный | Энтропия + компенсаторный polyE + CAASM → лимит делений. Stress integrator. | 🏃 **Фаворит** — без защиты | GLA |
...
**Прямой тест возраста центриоли (предложение D. Meyer, CECAD, 13 авг 2026 — принято):** pulse-label центриолей через асимметричные деления → U-ExM метрики M1–M5 отдельно для старой (материнской) и новой (дочерней) центриоли в одном и том же клеточном фоне. Предсказание: повреждения отслеживают физический возраст отдельной центриоли, а не возраст культуры. Это самый дешёвый и прямой первый тест центральной посылки; добавлен в Фазу 0 до основных экспериментов.

**Ограничение картуэлла/SAS-6 (верифицировано 2026-08-13):** картуэлл разбирается при созревании и в митозе (Huang 2025, PMID 39614048) и теряется при цилиогенезе в постмитотических нейронах C. elegans (Li 2017, PMID 28743734; Serwas 2017, PMID 28411189). Поэтому SAS-6 непригоден как маркер персистентной структуры в зрелых клетках — мишенями служат персистентные компоненты: триплет-микротрубочки, внутренний скэффолд (Le Guennec 2020, PMID 32110738), appendage-кольца.
...
| CAASM меняется с асимметричными делениями | Центросомный BioID + quantitative proteomics |
...
### Центриолярная архитектура (верифицировано через PubMed 2026-08-13)
| PMID | Автор, год | Ключевой результат |
|:----:|------------|-------------------|
| 28743734 | Li et al. (2017) | Картуэлл/SAS-6 теряется при цилиогенезе в нейронах C. elegans |
| 28411189 | Serwas, Dammermann (2017) | Центриоли диспенсабельны для созревания/поддержания цилии |
| 39614048 | Huang, Zhang (2025) | Визуализация разборки картуэлла в митозе |
| 32110738 | Le Guennec et al. (2020) | Внутренний скэффолд (cryo-EM, ~4 нм) — мишень M1 |
| 30559430 | Gambarotto et al. (2019) | U-ExM протокол (исправленный PMID) |
| 38442864 | Uzbekov, Avidor-Reiss (2024) | Возраст центриоли — биологический параметр |
...
*CONCEPT v4.6 — 2026-08-13. v4.5 + Meyer-тест (pulse-labeling mother-vs-daughter, принят 13 авг), картуэлл/SAS-6 ограничение, PMID верифицированы через PubMed.*
