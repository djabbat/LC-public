# MCARA — Мультисчётная Архитектура Старения Организма

**MCARA** — это теоретический мета-фреймворк, формализующий старение организма как взвешенную сумму нескольких параллельных процессов накопления повреждений («счётчиков»). Каждый счётчик имеет собственную кинетику, зависящую от делений клеток и хронологического времени, и фиксированные *априорные* весовые коэффициенты для каждой ткани, что обеспечивает фальсифицируемость. MCARA не заменяет специфические подпроекты (CDATA, Ze, BioSense), а предоставляет общий формальный язык и архитектуру для их интеграции.
...
* **Фальсифицируемость — первоклассная цель:** Каждое утверждение, выведенное из MCARA, должно сопровождаться чётким экспериментальным тестом, который может его опровергнуть (Аксиома M4).

## Ключевые компоненты

1. **Канонические счётчики:** MCARA определяет пять основных счётчиков: (1) центриолярная полиглутамилизация (CDATA), (2) теломеры, (3) митохондриальные ROS/мтДНК, (4) эпигенетический дрейф, (5) коллапс протеостаза.
...
* **[THEORY.md]():** Полное формальное изложение аксиом, уравнений и предсказаний MCARA.
* **[EVIDENCE.md]():** Проверенные литературные источники (PMID/DOI), внутренние данные и опровергающие свидетельства, поддерживающие или оспаривающие MCARA.
* **[OPEN_PROBLEMS.md]():** Неразрешённые научные проблемы, приоритеты и конкретные фальсифицируемые тесты для MCARA.
* **[PARAMETERS.md]():** Таблица всех количественных параметров, их источников, единиц измерения и статуса калибровки.
* **[DESIGN.md]():** Архитектура кода, файловая структура и API контракты эталонной реализации на Rust.
* **[AGENTS.md]():** Инструкции и жёсткие правила для LLM-агентов, работающих с кодом и документацией MCARA.
* **[JOURNAL.md]():** Хронологический журнал изменений, решений и их обоснований.
* **[ROADMAP.md]():** План будущих разработок, приоритеты и зависимости.

## Важные канонические исправления (2026-04-22)

* **Формула Health Score удалена.** Веса 0.40·organism + 0.25·psyche... не имели математического обоснования из MCARA. Вместо неё используется прямая тканевая нагрузка `L_tissue`.
...
* **Связь γ_i и MCARA Test 2.** Параметр связи `γ_i` по умолчанию равен 0. MCARA Test 2 — это будущий протокол для *измерения* связей между уже работающими счётчиками, а не источник *априорных* значений `γ_i`.
* **Структура EIC Pathfinder.** В текущей заявке (Variant B) MCARA является WP1 (€0.3M, M1-M12). Подпроекты Ze, BioSense и Activatus не включены в качестве отдельных рабочих пакетов.
