...
**Аксиома M4 (Фальсифицируемость как принцип первого порядка).** Любое утверждение, дедуктивно выведенное из MCARA, должно сопровождаться описанием практически выполнимого экспериментального теста, результат которого может это утверждение опровергнуть. Наличие таких тестов является обязательным атрибутом завершённой теоретической конструкции в рамках MCARA.
...
## 4. Канонический набор счётчиков MCARA (v1.0)
...
**Therapeutic relevance:** 9 piRNA идентифицированы как potential therapeutic targets (Kraus 2026); GLP-1-индуцированные сдвиги piRNA-профилей и lifestyle-interventions (exercise) активно изучаются. Требуют независимого подтверждения до операционализации в MCARA-протоколе.
...
**Следствие для MCARA:** GrimAge — наилучший integrative marker мастер-счётчика; его использование в качестве primary endpoint оправдано, **но** см. damage shadow rule (THEORY §4.4): эпигенетический reset не = функциональное омоложение.
...
**Hierarchical model rejuvenation potential** (поддерживает MCARA pluralism):
...
**Прямое следствие для MCARA:** DNAmAge **не валидный surrogate endpoint** для системных функциональных исходов; необходимо parallel functional validation. Подтверждает M1: single-counter intervention (только #4) недостаточна для омоложения.
...
**Ключевое правило:** Элементы `Γ_{ij}` (и, следовательно, `γ_i` в упрощённой форме) должны **измеряться** в контролируемых экспериментах (см. MCARA Test 2), а не быть свободными параметрами для подгонки. Это отделяет причинно-следственный вывод от корреляционного.

## 6. Предсказания теории MCARA
