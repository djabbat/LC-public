...
Модель Proteostasis реализована как **Python-пакет `proteostasis`**, который является подмодулем общей вычислительной платформы MCARA. Архитектура следует принципам **модульности, проверяемости и воспроизводимости**. Модель отделена от данных, а все параметры вынесены в конфигурационные файлы.
...
class ProteostasisCounter:
 """
 Модель MCARA Counter #5 (Collapse of Proteostasis).
 """
...
* **Выходные данные модели:** Модель возвращает скаляр или массив `D5`. Для интеграции в MCARA, вызывающий код использует тканеспецифичный вес `w₅` из общего конфигурационного файла MCARA.
