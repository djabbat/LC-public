...
The fundamental MCARA equation for Epigenetic Drift is:
...
* ***γ₄ · I(other counters)***: **Coupling term.** Represents the summed input from the states of other MCARA counters, scaled by a coupling coefficient *γ₄*. *I(·)* is an interaction function (initially modeled as linear summation).
...
## 5. Coupling (Γ Matrix) with Other MCARA Counters

A core tenet of MCARA is that counters interact. The influence of other counters on the rate of Epigenetic Drift is defined by off-diagonal elements in the MCARA coupling matrix Γ. Below are the candidate couplings for Γ₄,ⱼ:

* **Γ₄,₁ (Centriolar → Epigenetic):** **Hypothesis - Measurement Pending.** The primary cilium is a signaling hub. Centriolar dysfunction (Counter #1) could disrupt cilium-dependent signal transduction (e.g., Hedgehog, Wnt), pathways known to regulate chromatin modifiers and gene expression programs during cell fate decisions. This coupling is plausible but currently unquantified; it is marked for measurement in ~~MCARA Test 2 (withdrawn — see CORRECTIONS_2026-04-22.md)~~ [отозвано — see CORRECTIONS §1.3].
...
MCARA Test 1 (Tissue-Specific Dominance):
...
MCARA Test 2 (Coupling Independence) – отозвано, но для γ₄ⱼ:
...
For Counter #4 to be a valid component of MCARA, it must satisfy specific, quantitative falsifiability conditions (MCARA Tests 1 & 2).

* **MCARA Test 1 (Tissue-Specific Dominance):** The counter must demonstrate a monotonic increase with age in relevant tissues and its parameters (α₄, β₄) must align with tissue proliferative status.
...
## 7. Integration within the MCARA Framework

In the full MCARA model, the aging state of a tissue is an emergent property of all counters:
...
This formalization highlights several critical unresolved issues that define the frontier of research on epigenetic aging and its role in MCARA:
...
The primary analyses for testing the falsification conditions of Counter #4 will be pre-registered on the Open Science Framework. Placeholder: https://osf.io/TBD. Planned registration date: 2026-12-01. The pre-registration will specify: (1) the exact statistical models for each condition (linear regression for β₄, mixed-effects for α₄, structural equation model for γ₄ⱼ), (2) the primary outcome variables (DunedinPACE for β₄, replication-associated methylation changes for α₄, coupling coefficients from MCARA integration), (3) the covariates (age, sex, tissue type, batch effects), (4) the exclusion criteria (outliers >3 SD, missing >20% CpG sites), and (5) the stopping rule (N determined by power analysis, see Sample size calculation). Exploratory analyses (e.g., tissue-specific effects, sex differences) will be clearly marked as such.
...
| ABL-2 primacy undermines counter independence | Medium | 4 | Explicitly model ABL-2 as upstream parameter; test via conditional independence in MCARA | Low |
...
The primary confirmatory analysis for MCARA Test 1 (tissue‑specific dominance) will be pre‑registered at the Open Science Framework (OSF) prior to data collection.
