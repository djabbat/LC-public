...
## 16. New Evidence — Oxygen Paradox & Aging-Clock Integration (2026-08-12)

### Кислородный парадокс (для Aubrey de Grey proposal)

**Parrinello et al. 2003 (PMID 12855956, Nat Cell Biol) — верифицировано:**
- MEF в **3% O₂** (физиологический) — НЕ сенесцируют в стандартных условиях эксперимента
- MEF в **20% O₂** — накапливают повреждения ДНК → «спонтанное событие» → **бесконечная пролиферация (иммортализация)**
- Вывод: **иллюзия отсутствия предела делений в 21% O₂** — артефакт оксидативного стресса; настоящий предел виден только в физиологическом кислороде
...
| Статья | PMID | Роль Meyer | Суть |
|--------|------|-----------|------|
| BiT age, Aging Cell 2021 | 33656257 | **1-й автор** | Транскриптомные часы, бинаризация, почти теоретический предел точности; C. elegans + человек |
| Aging clocks stochastic variation, Nat Aging 2024 | 38724736 | **1-й автор** | Накопление стохастической вариации достаточно для построения часов |
| The stochasticity of biological aging, Nat Med 2025 | 40301558 | **1-й автор** | Старение = накопление случайности |
| Aging by the clock and yet without a program, Nat Aging 2025 | 40968181 | **1-й автор** | Часы есть, программы нет |
| Gallrein, Meyer, Nat Aging 2026 | 41634462 | соавтор (равный) | Часы различают уязвимые/устойчивые нейроны C. elegans |
| Koch, Meyer, Nat Aging 2026 | 42230995 | соавтор | DREAM репрессирует репарацию ДНК → мутации → lifespan |

**Ключевой синтез:** Meyer измеряет стохастическую вариацию (часы); CEDAR предлагает её физический источник (центриоль). Proposal соединяет «что» (часы) с «почему» (центриоль).

### Мостик центриоль → p53/p21/DREAM (собран из PMID)

```
Центриолярная дисфункция (потеря/амплификация) → p53 (Meitinger 2016, PMID 27432897)
  → p21 → DREAM-ось сенесценции (Schmidt 2024, PMID 38783095)
  → DREAM репрессирует репарацию ДНК → соматические мутации (Koch 2026, PMID 42230995)
  + PIDDosome-инфламмация (Garcia-Carpio 2023, PMID 37530438)
  + автофагия (Coelho 2026, PMID 42324259)
```
Структурное повреждение — третий класс дисфункции (потеря/амплификация/повреждение); тестируется напрямую в Эксперименте A.0 (STOP-rule).

### Peer-reviewed маркеры SDA (замена препринта Yang)

- **CEP128** — Kashihara 2019 (Genes Cells, PMID 30623524) — «Cep128 associates with Odf2 to form the subdistal appendage»
- **ninein** — Tillery 2024 (Mol Biol Cell, PMID 39024292)
- **NDE1/NDEL1** — Inaba 2016 (JCB, PMID 26880200)
- **Woglar 2022** (PLoS Biol, PMID 36107993) — U-ExM+STED архитектура центриоли C. elegans (критично для Эксперимента A)

### Proposal файлы (архив)

- `docs/proposals/Proposal_Meyer_CEDAR_aging_clocks.md` — v3.6 (8 циклов рецензий: 68→~93)
- `docs/proposals/Testing_the_Hypothesis_of_Centriolar_Entropy_as_a_Source_of_Transcriptomic_Aging_Clock_Signal.md` — формат research article
- `docs/proposals/Proposal_Aubrey_Centriole_Reset.md` — v2 (кислородный парадокс)

## 17. GUICHARD Feedback — SAS-6 Cartwheel Disassembly in Vertebrates (2026-08-13)

### Критический экспертный ответ (Paul Guichard, UNIGE — cryo-ET центриолей)

**Ответ на письмо «SAS-6 cartwheel geometry and centriole ageing» (12.08, отправлено 21:41 Tbilisi):**

1. **В человеческих/позвоночных центриолях картвел SAS-6 ДИЗАССЕМБЛИРУЕТСЯ при созревании.** Присутствует в процентриолях/молодых центриолях; **отсутствует в зрелых материнских центриолях**.
   → **Следствие:** SAS-6 ring geometry НЕ может быть readout возраста зрелой человеческой центриоли.
2. **В C. elegans картвел ПЕРСИСТИРУЕТ** через клеточные циклы (работа Gönczy — подтверждено Guichard).
   → **Следствие:** Эксперимент A (C. elegans, SAS-6 AID-деградация) ВАЛИДЕН и усилен — картвел червя — идеальный readout.
3. **U-ExM:** недостаточное разрешение для тонких изменений геометрии картвела (диаметр, pitch, спицы) → STED/крио-ЭТ на подвыборке.
4. **Совет Guichard:** персистентные структуры зрелой центриоли как цели: **триплетные микротрубочки, внутренний скаффолд (inner scaffold), A-C линкер**.

### Изменения в дизайне (все документы обновлены)

| Документ | Изменение |
|----------|-----------|
| Proposal Meyer v3.6 | M1 → Persistent-scaffold geometry (триплеты, inner scaffold POC1A/B/POC5, A-C линкер); картвел — C. elegans/процентриоли only; U-ExM resolution caveat |
| Research Article | M1 переопределён аналогично; STED/крио-ЭТ верификация |
| Proposal Aubrey v2 | Список метрик обновлён (без SAS-6 cartwheel для зрелых) |
| Эксперимент A (C. elegans) | УСИЛЕН — картвел персистирует (Guichard + Gönczy) |

### Follow-up (отправлен 13.08 ~00:04)

Вопрос Джабы: реалистичны ли inner scaffold + A-C линкер как количественные цели между early/late-passage (или та же проблема с томограммами)? + Приглашение на Кёльн 31.08–02.09 (возможно удалённо).

### Урок

Экспертные консультации до регистрации метрик окупаются: рецензенты этого бы не заметили, но эксперимент был бы невалиден. Письмо Guichard — модель работы с экспертами (короткое, конкретное, с действием).

*Files: refs/Guichard_SAS6_cartwheel_critical_reply_2026-08-12.md; letters/sent/2026-08-12_Guichard_SAS6_cartwheel_aging.md*

## 18. GEO PILOT — Centriolar Genes Respond to Aging/Senescence (2026-08-13)

**GSE293225** (HdFbs 12P vs 20P × D-ribose 0/15/30 мM): первый эмпирический пилотный сигнал Phase 0.

| Сравнение | CEP128 | SAS6 | POC1A | NIN | POC5 |
|-----------|--------|------|-------|-----|------|
| 20P/12P (пассаж) | **1.40 ↑** | **1.25 ↑** | **1.16 ↑** | 0.83 ↓ | 0.87 ↓ |
| ribose30/0 (стресс) | **0.62 ↓** | **0.72 ↓** | **0.61 ↓** | 1.04 | 1.17 |
...
## 21. Benign Tumors (Nevi) — Model of the Counter System (2026-08-13)

**Верифицировано:** Lorbeer 2024 (PNAS Nexus, PMID 38371417 — диспластические невусы: разные механизмы сенесценции); Michaloglou 2005 (Nature, PMID 16079850 — BRAF-невусы = OIS); Serrano 1997 (Cell, PMID 9054499 — OIS классика); Wynford-Thomas 1999 (J Pathol, PMID 10341711).

**Аналогия с побегом фибробластов:**
- **Невус** = побег, остановленный счётчиком (p16/p53 активны → сенесценция; теломераза низкая)
- **Побег фибробластов** = счётчик сломан (p53/p19ARF инактивированы + теломераза/ALT)
- **Рак** = все счётчики сломаны

**Польза:**
- **CEDAR:** невусы — естественный эксперимент; предсказание: центриолярная глубина повышена в невусах, сброшена в меланоме (тестируемо на архивных образцах IF/U-ExM)
- **LERR:** безопасная управляемая остановка существует (невусы); обратимость OIS при снятии p16/p53; Rebuild должен избегать путей прогрессии (p16/p53 потеря + теломераза)
- **MCARA:** иерархия счётчиков видна (теломеры + OIS + центриолярный); Lorbeer 2024 («разные механизмы») согласуется с 5-счётчиковой архитектурой

**Файл:** `docs/proposals/BENIGN_TUMORS_LERR_MCARA_CEDAR_2026-08-13.md`
