...
- Computational modeling & MCARA integration: [Your lab/group].
...
* **[THEORY.md](./THEORY.md)** — Полная формальная спецификация: аксиомы, вывод основного уравнения, математические предсказания и интерпретация параметров в рамках MCARA.
...
* **[OPEN_PROBLEMS.md]()** — Критические нерешённые вопросы, такие как количественная оценка константы времени `τ₂` и разделение вкладов `α₂` и `β₂` in vivo. Для каждой проблемы приведены тесты на фальсифицируемость с чёткими критериями.
...
* **[AGENTS.md]()** — Инструкции для ИИ-агентов (например, для анализа литературы или планирования экспериментов) с жёсткими правилами безопасности и ссылками на канонические определения.
* **[JOURNAL.md]()** — Хронологический журнал изменений, решений и их обоснований в рамках этого подпроекта.
* **[ROADMAP.md]()** — План будущих работ: приоритетные задачи, зависимость от других подпроектов (например, CEDAR для валидации), этапы интеграции.

## Контекст и ограничения

* **В рамках MCARA:** Теломерный счётчик — один из нескольких (наряду с центриолярным, митохондриальным ROS, эпигенетическим дрейфом). Его вклад `w₂(tissue)` варьируется между тканями и должен определяться калибровкой на данных.
...
* **Три аксиомы CEDAR:** Если проект рассматривается в контексте CEDAR (Cellular Damage & Telomere Attrition), то три его аксиомы (1. Повреждение накапливается, 2. Теломеры — счётчик делений и стресса, 3. Сигнальные пути интегрируют повреждение) считаются ненарушимыми для целей данного подпроекта.
