# Formal Theory of the Telomere Shortening Counter (MCARA #2)

## 1. Аксиоматическая основа в MCARA

Теломерный счётчик определяется в рамках следующих аксиом Multi-Counter Architecture of Organismal Aging (MCARA):
...
* Член `γ₂ · I(others)`: Место для функции, описывающей направленное влияние других счётчиков MCARA на скорость изменения `D₂`. Явный вид этой функции определяется матрицей связей Γ (см. раздел 4).
...
## 4. Связи с другими счётчиками MCARA (Матрица Γ)
...
Эти предсказания поддаются количественной проверке, как описано в **[OPEN_PROBLEMS.md]()**.
