...
Теломерный счётчик реализован как независимый модуль (класс `TelomereCounter`) в рамках симуляционной платформы MCARA. Его основная задача:
...
```
mcga_framework/ # Корень проекта MCARA
├── counters/
│ ├── __init__.py
...
class TelomereCounter:
    """
    Implements the telomere shortening counter (Counter #2) within the MCARA framework.
    """
