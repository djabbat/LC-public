...
6. **Конфликт нумерации Counter #2 с CEDAR не разрешён**  
   Упоминается в `CLAUDE.md` как P0 finding. Это блокирует интеграцию в MCARA.
...
4. **Недостаток системного проектирования**  
   Архитектура была спущена сверху (MCARA), но детали реализации (как связать crate, backend, frontend) не проработаны. Зависимости между модулями не специфицированы.
