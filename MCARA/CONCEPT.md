...
| Организм | Изменение центриоли | Изменение судьбы клетки | PMID |
|----------|---------------------|------------------------|:----:|
| C. elegans | Элиминация в >98% соматических клеток при терминальной дифф-ке | Выход из клеточного цикла | 37963546, 40475707 |
| Drosophila ♀ | Принудительное удержание материнских центриолей | Тотипотентность нарушена, стерильность | 27229142 |
| Планарии | De novo появление центриолей при терминальной дифф-ке | Пластичность потеряна | 32776587 |
| Человек (hPSC) | PLK4↓ → потеря центриолей | Плюрипотентность → p53-зависимая дифф-ка | 30197118 |

Каждая запись — корреляция, не демонстрация причины. Но одна и та же корреляция в четырёх независимых линиях вряд ли случайна.
...
**Предсказание (плюрипотентность):** Элиминация центриолей → эффективность репрограммирования ↑ ≥2× vs контроль.
**Гипотеза (тотипотентность, 2026-07-12):** Центриоль = **два режима:** (1) пассивный накопитель энтропии — основа старения многоклеточных животных (снижение функции центросомы и цилии → падение самообновления тканей); (2) активный регулятор дифференцировки — инструмент необратимого переключения генных сетей (DID-РНК, CAMC, NANOG, cilium). Центриоль стареет **по второму закону термодинамики** (как любая вещественная структура — белки, липиды, все молекулы), а не от делений и не от участия в дифференцировке — это верно для любой клетки, включая постмитотические нейроны. **Энтропия — не храповик. Храповик — переключение генных сетей.** Необратимая дифференцировка = необратимое отключение одних генных сетей и включение других. Центриоль — ИНСТРУМЕНТ этого переключения (через DID-РНК, CAMC, NANOG, cilium), а не пассивный накопитель энтропии. Старение организма — результат накопления старых центриолей в стволовых клетках, снижающего способность к самообновлению. Асимметричные деления продвигают храповик на один щелчок: старая центриоль → стволовая клетка (сохраняет статус), новая центриоль → дифференцирующаяся клетка (шаг вперёд). Стволовые клетки накапливают старые центриоли, а не избавляются от них. Разные центриоли несут разные CAMC-профили: старая удерживает стволовое состояние, новая — толкает в дифф-ку. Внутри CAMC — DID-частицы: при делении одна отщепляется → уходит с новой центриолью; при редупликации — с уменьшением. **DID = РНК** (Tqemaladze, 2026-07-12): DID-РНК → обратная транскриптаза → ДНК → встройка в центромеру/повторы → отключение одной генной сети + включение другой. LINE1/HERV — «одомашненные» потомки. Старение организма — цена за истинную дифференциацию (в отличие от модуляции у растений). **PolyE и подобные молекулы — КОМПЕНСАЦИЯ дисфункции центриоли**, в том числе от накопления энтропии. TTLL добавляют polyE для стабилизации; CCP удаляют при восстановлении. Аутофагия может разрушать центриоли (C. elegans), но у соматических клеток C. elegans нет сброса генных сетей (GRN reset) — поэтому элиминация без Reprogram и Rebuild фиксирует текущий статус, а не создаёт тотипотентность. Все три шага обязательны. **Барьер = DID + старая центриоль с GRN в нетотипотентном режиме + начало de novo синтеза (момент переключения GRN).** Бипотентные состояния in vitro достигаются из плюрипотентных клеток (движение вниз). Движение вверх от соматической клетки — против храповика. **Необратимая дифференцировка = необратимое отключение одних генных сетей и включение других. В наивных клетках (ES, iPSC) все генные сети открыты — поэтому TLSC достигаются химически без центриольной элиминации. В соматических клетках сети закрыты — для их открытия нужен сброс храповика.** Альтернатива: maturity sensor (Lindhout 2021) — центриоль = датчик зрелости, не замок. Потеря снижает порог дифф-ки неспецифически. Обе модели предсказывают ↑ пластичности после элиминации. Различающие тесты: (а) ядерный NANOG ↑ → активный регулятор; (б) элиминация без факторов → хаотичная ZGA → maturity sensor; (в) IFT88 shRNA → если cilium-потеря имитирует центриоль-потерю → эффект через cilium. Модели не взаимоисключающие. OSKM перезагружает software (эпигеном), но не hardware (центриоль). Полная элиминация центриолей снимает физический замок дифференцировки. Предсказание: (1) OSKM после элиминации → ≥2× эффективность + возможно промежуточное состояние между плюри- и тотипотентностью; (2) тотипотентные факторы (DUX4 + TPRX1) после элиминации → настоящая тотипотентность с MERVL+, Zscan4+, CDX2+. Маркёры: MERVL, Zscan4, Hhex (2C-like мышь); TPRX1, ZSCAN4, DUX4 (человек). В природе тотипотентность всегда сопровождается элиминацией центриолей — ооцит очищает свои центриоли, сперматозоид приносит лишь seeds для de novo сборки.
