...
**Synthesized merge (planned for Q3 2026):** unifies the CEDAR theoretical framework with CellLineageTree experimental design and a single integrated experimental platform.
...
## §2 Theoretical framework (CEDAR Counter #1 in MCARA)

The CEDAR model formalizes centriolar damage as:

\[
D_{\text{centriole}}(n, t) = D_{\text{centriole},0} + \alpha \cdot (n / n^*) + \beta \cdot (t / \tau) + \gamma \cdot I(\text{other counters})
\]

where \(n\) = mitotic divisions, \(t\) = chronological time, \(n^*\) and \(\tau\) are characteristic scales, and \(I(\text{other})\) couples to MCARA counters #2–5 (telomere, mitochondrial ROS, epigenetic drift, proteostasis).

### §2.1 Core predictions (consolidated)

The following predictions are the primary testable outputs of the CEDAR model and form the basis of the experimental design. All predictions are stated as hypotheses to be tested; no pilot data currently exist to support them except where explicitly noted.
...
- **P7:** CCP1-knockdown phenocopies aged-mother phenotype (osteogenic differentiation defect). *This prediction is currently hypothetical; validation is planned in ongoing experiments. The prediction is retained as a speculative extension of the CEDAR framework but is not used as independent evidence in this proposal. If published data become available prior to Phase A start, this prediction will be upgraded to a supporting prediction.*
...
- **Prediction D3:** The lineage tree platform will directly measure de novo event frequency. If de novo events exceed 5% of all divisions, the primary endpoint (asymmetry rate) will be adjusted to exclude divisions preceded by a de novo event, and the CEDAR model will be revised to incorporate a stochastic reset probability. If de novo events exceed 20% of divisions, CEDAR’s core mechanism (accumulated damage on mother centriole as primary driver) would be considered falsified because the damage accumulation would be diluted faster than it could drive aging.
...
A critical challenge for the CEDAR model is distinguishing whether centriolar polyGlu accumulation is a primary driver of aging or a downstream consequence of other aging processes (e.g., reduced proteostasis, mitochondrial dysfunction). To address this, we propose the following experimental strategy:
...
Beyond the de novo synthesis frequency condition (§2.1), the CEDAR model will be considered falsified if any of the following are observed:
