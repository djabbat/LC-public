...
def test_stuck_signature_normalizes_search_tools():
    """Search-type calls that differ only in volatile params (k/limit) or
    case collapse to the same signature so the loop guard catches them."""
    from agents.generalist import _stuck_signature
    a = {"tool": "memory_recall", "args": {"query": "TBPR Cycle 1", "k": 5}}
    b = {"tool": "memory_recall", "args": {"query": "tbpr cycle 1", "k": 10}}
    c = {"tool": "memory_recall", "args": {"query": "  TBPR Cycle 1  "}}
    assert _stuck_signature(a) == _stuck_signature(b) == _stuck_signature(c)


def test_stuck_signature_keeps_distinct_queries():
    """Genuinely different queries still get distinct signatures."""
    from agents.generalist import _stuck_signature
    a = {"tool": "memory_recall", "args": {"query": "TBPR Cycle 1"}}
    b = {"tool": "memory_recall", "args": {"query": "FCLC peer review"}}
    assert _stuck_signature(a) != _stuck_signature(b)


def test_stuck_signature_non_dedup_tools_strict():
    """Tools not in _SEMANTIC_DEDUP_TOOLS keep strict signatures —
    write_file at content=A vs content=B must stay distinct."""
    from agents.generalist import _stuck_signature
    a = {"tool": "write_file", "args": {"path": "/tmp/x", "content": "A"}}
    b = {"tool": "write_file", "args": {"path": "/tmp/x", "content": "B"}}
    assert _stuck_signature(a) != _stuck_signature(b)


def test_stuck_loop_flattens_multi_action():
    """Multi-action blocks `{actions: [...]}` and parallel blocks
    `{parallel: [...]}` flatten into per-step entries in recent_actions
    so a single LLM round emitting 3 read_file's of the same file still
    triggers window_hits >= 3."""
    from agents.generalist import _stuck_signature, _SEMANTIC_DEDUP_TOOLS
    # Simulate the runtime flattening
    same_read = {"tool": "read_file", "args": {"path": "/foo.md", "limit": 500}}
    same_read_alt = {"tool": "read_file", "args": {"path": "/foo.md", "limit": 1000}}
    multi = {"actions": [same_read, same_read_alt,
                          {"tool": "read_file", "args": {"path": "/foo.md"}}]}
    inner: list[dict] = []
    if isinstance(multi.get("actions"), list):
        for step in multi["actions"]:
            inner.append(step)
    sigs = [_stuck_signature(s) for s in inner]
    # All three inner read_file's of /foo.md should collapse to one sig
    assert sigs[0] == sigs[1] == sigs[2]
    # Recent_actions count would be 3 in last 3 = window_hits >= 3 → trigger
    assert sigs.count(sigs[0]) == 3


def test_stuck_signature_read_file_collapses_offset_limit():
    """read_file/view_file on the same path collapse regardless of
    offset/limit/start_line/end_line — repeatedly re-reading the same
    file with varying ranges is the same kind of fruitless loop as
    repeated memory_recall with rephrased queries."""
    from agents.generalist import _stuck_signature
    a = {"tool": "read_file", "args": {"path": "/foo.md", "limit": 500}}
    b = {"tool": "read_file", "args": {"path": "/foo.md", "offset": 0, "limit": 200}}
    c = {"tool": "view_file", "args": {"path": "/foo.md", "start_line": 1, "end_line": 132}}
    d = {"tool": "view_file", "args": {"path": "/foo.md"}}
    # same path, same kind-of tool → collapse
    assert _stuck_signature(a) == _stuck_signature(b)
    assert _stuck_signature(c) == _stuck_signature(d)
    # different paths still distinct
    e = {"tool": "read_file", "args": {"path": "/bar.md"}}
    assert _stuck_signature(a) != _stuck_signature(e)


def test_stuck_loop_read_file_relaxed_threshold():
    """Read-class tools (read_file/view_file) need ≥4 hits in the window
    to trigger, not 3 like search tools — legitimate file paging shouldn't
    abort. Reproduces the false-positive reported by user 2026-05-09 where
    `read_file X` then `read_file X` (only 2 calls) fired the abort."""
    from agents.generalist import _READ_CLASS_TOOLS, _SEMANTIC_DEDUP_TOOLS
    assert "read_file" in _READ_CLASS_TOOLS
    assert "memory_recall" not in _READ_CLASS_TOOLS

    # Two read_file's of same file should NOT trigger (window_hits=2 < 4)
    actions = ["read_file|/foo.md", "read_file|/foo.md"]
    sig = "read_file|/foo.md"
    threshold = 4 if "read_file" in _READ_CLASS_TOOLS else 3
    assert actions[-6:].count(sig) < threshold  # 2 < 4: no trigger

    # Three read_file's of same file STILL should NOT trigger for read class
    actions = ["read_file|/foo.md", "read_file|/foo.md", "read_file|/foo.md"]
    assert actions[-6:].count(sig) < threshold  # 3 < 4: no trigger

    # Four read_file's WILL trigger
    actions = actions + ["read_file|/foo.md"]
    assert actions[-6:].count(sig) >= threshold  # 4 >= 4: trigger

    # But for memory_recall (search class), 3 IS the threshold
    threshold_mr = 4 if "memory_recall" in _READ_CLASS_TOOLS else 3
    assert threshold_mr == 3
    actions = ["memory_recall|q1", "memory_recall|q1", "memory_recall|q1"]
    assert actions[-6:].count("memory_recall|q1") >= threshold_mr


def test_search_spam_window_count():
    """Frequency cap: same search tool called ≥4 times in last 6 actions
    with cosmetically-different queries triggers the loop guard.
    Reproduces the real failure mode where memory_recall is called with
    "TBPR Cycle 1", then "TBPR review next steps", then "spisok TBPR" —
    each query string differs but it's the same fruitless search loop."""
    from agents.generalist import _SEMANTIC_DEDUP_TOOLS
    assert "memory_recall" in _SEMANTIC_DEDUP_TOOLS

    # Simulate the runtime detection logic
    recent_tool_names = ["memory_recall", "glob", "memory_recall",
                          "memory_recall", "read_file", "memory_recall"]
    tool = "memory_recall"
    search_spam = (tool in _SEMANTIC_DEDUP_TOOLS and
                    recent_tool_names[-6:].count(tool) >= 4)
    assert search_spam is True

    # Mixed workflow with 3 different searches should NOT trigger
    recent_tool_names2 = ["memory_recall", "glob", "memory_recall",
                           "read_file", "grep", "memory_recall"]
    spam2 = (tool in _SEMANTIC_DEDUP_TOOLS and
              recent_tool_names2[-6:].count(tool) >= 4)
    assert spam2 is False
