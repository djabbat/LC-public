...
# Open Problems and Falsifiable Tests for MCAOA
...
**Priority:** **High** (P0). Blocks full experimental validation of MCAOA.
...
**Possible Outcomes:**
1.  **✅ Strong correlation (R² > 0.7):** Confirms the possibility of *a priori* weight prediction. MCAOA passes this test.
...
4.  **🔀 Contradictory result:** Different prediction methods yield widely varying `w_i_pred`. Indicates fundamental uncertainty in the selection of *a priori* parameters, which weakens the entire MCAOA framework.
...
**Possible Outcomes:**
1.  **✅ Slowing of all counters and phenotypes:** Strongly supports an upstream role for CP. MCAOA with CP #1 is confirmed.
...
4.  **🔀 Tissue-specific effect:** Slowing observed only in the highly proliferative intestinal epithelium, but not in the liver. Confirms a context-dependent role for CP, which is consistent with MCAOA but complicates the overall theory.

## Problem 3: Measuring the Connection Matrix Γ and Testing the Hypothesis `γ_i = 0`

**Description:** By default, MCAOA assumes `γ_i = 0` (counter independence hypothesis). Non-zero connections must be proven. There is no standardized in vivo protocol for directly measuring the elements `Γ_{ij}`.

**Priority:** **Medium** (P2). Critical for predicting intervention synergy.

### Falsifiable Test 3A: MCAOA Test 2 Protocol (in vitro calibration)
...
4.  **🔀 Non-linear dependence:** The effect is observed only after a certain threshold of mitochondrial damage. Would require modification of the linear connection model in MCAOA to a threshold or sigmoidal model.
...
**Hypothesis:** In a fibroblast culture where the telomere counter dominates, the MCAOA-computed load `L_tissue` will increase monotonically with passages and will reach a constant value `L_critical` at the moment of entry into senescence (Hayflick limit).
...
## Problem 5: piRNA Counter #6 — Refuting Evidence and Exploratory Status

*Updated 2026-05-22: Downgraded to EXPLORATORY after expert review. Refuting evidence added.*

**Description:** piRNA-mediated transposon repression was initially proposed as Counter #6. However, **direct refuting evidence** exists that challenges this hypothesis:

### Refuting Evidence #1: Piwi-гены необязательны для HSC мышей
- **PMID: 24058407** | Nolde et al. *Blood* 2013
- Мыши с глобальной делецией всех трёх piwi-генов (Miwi, Mili, Miwi2) сохраняют нормальное кроветворение
- HSC PIWI-дефицитных мышей способны к нормальной реконституции после трансплантации
- Эффект отсутствует как в гомеостазе, так и после миелоаблативного стресса (5-FU)
- **Вывод для MCAOA:** piRNA-путь не является критическим для HSC, по крайней мере у мышей

### Refuting Evidence #2: PIWIL2 как маркер плохого прогноза при MDS
- **PMID: 39849644** | *Biomarker Research* 2025
- PIWIL2 сверхэкспрессирован при MDS высокого риска
- Связь с **худшей выживаемостью**, а не защитой
- Транспозоны и piRNA параллельно дерегулированы
- **Вывод для MCAOA:** Роль piRNA-пути в гемопоэзе сложнее, чем простой «счётчик повреждений»

### Ограничение: piRNA-независимые функции PIWI
- **PMID: 39981094** | Claro-Linares et al. *Front Cell Dev Biol* 2025
- В кишечных стволовых клетках *Drosophila* PIWI белок Piwi активен, но **piRNA отсутствуют**
- PIWI-белки работают на хроматиновом и пост-транскрипционном уровнях независимо от piRNA
- **Вывод для MCAOA:** Knockdown PIWI (Arm F) может нарушать регуляцию через piRNA-независимые механизмы, что искажает интерпретацию

### Дополнительные ограничения (из литературы)
- Sturm 2017 [PMID:28653810] — Piwi-piRNA в **Drosophila и Hydra**, не в HSC человека
- Parambil 2022 [PMID:38142432] — piRNA генерация в **HEK293**, не в HSC
- Kraus 2026 — циркулирующие piRNA предсказывают выживаемость, но **корреляция ≠ причинность**

### Статус
**EXPLORATORY.** Не включён в основной дизайн Phase III. Для канонического включения требуется:
1. Прямое доказательство, что piRNA-путь критичен для HSC человека (опровержение Nolde 2013)
2. Демонстрация причинной роли (не корреляционной) piRNA в старении HSC
3. Разделение piRNA-зависимых и piRNA-независимых функций PIWI
...
**Priority:** **High** (P1). Blocks the valid use of DNAmAge as a surrogate endpoint in MCAOA-driven trials.
...
**Possible Outcomes:**
1.  **✅ ΔD_shadow ≈ 0 with ΔDNAmAge < 0:** confirms damage shadow as an epigenetically-independent counter group; mandates parallel functional + structural endpoints in MCAOA trials.
