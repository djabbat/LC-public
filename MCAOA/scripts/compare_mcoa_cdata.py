#!/usr/bin/env python3
"""compare_mcoa_cdata.py — MANDATORY MCAOA-vs-CDATA comparison harness.

Per project rule (feedback_mcoa_cdata_comparison): every MCAOA simulation run MUST be paired with
...
def compare(mcoa_csv: Path, cdata_csv: Path, tissue: str, label: str, out_dir: Path) -> Path:
    ...
    # CDATA exports a single 'damage' column (historical); MCAOA exports per-counter + tissue_load.
    ...
    with report_path.open("w") as f:
        f.write(f"# MCAOA vs CDATA comparison — {label}\n\n")
        f.write(f"**Date:** {date}\n")
        f.write(f"**Tissue:** {tissue}\n")
        f.write(f"**MCAOA input:** `{mcoa_csv}`\n")
        f.write(f"**CDATA input:** `{cdata_csv}`\n")
        f.write(f"**MCAOA column compared:** `{mcoa_col}`\n")
        ...
        f.write("- [ ] (b) artefact of MCAOA dimensionless normalisation\n")
...
def main():
    p = argparse.ArgumentParser(description="MANDATORY MCAOA-vs-CDATA comparison harness.")
