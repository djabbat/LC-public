...
Proteostasis) + MCAOA coordinator. Every full run produces all 15
pairwise counter-vs-counter residuals across 6 tissues = 90 cells.

Usage
-----
    cd ~/Desktop/LC/MCAOA/scripts
...
def main():
    ...
    out_dir = Path(args.out).expanduser().resolve() if args.out else \
        ROOT / 'MCAOA' / 'docs' / 'comparisons' / f'{dt.date.today().isoformat()}_full'
    ...
    md = [
        f'# MCAOA full comparison — {dt.date.today().isoformat()}',
        '',
