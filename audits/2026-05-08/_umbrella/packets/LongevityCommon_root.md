# AUDIT PACKET — LongevityCommon_root

Path: `/home/oem/Desktop/LongevityCommon`  Date: 2026-05-08

## Size & file counts
```
8,3G	/home/oem/Desktop/LongevityCommon
```
...
|-----------|--------------------|------------------------------------------------|--------|
| **MCOA** | Theoretical meta-framework | Cross-WP framework | submitted Nature Aging, NOT peer-reviewed |
...
| Telomere/MitoROS/EpigeneticDrift/Proteostasis | counter modules | mention only as "future MCOA counters" | concept-stage |
...
```
   ...
   └─────────────────────────┬───────────────────────────────┘
                             ↓
   ┌─────────────────────────┴───────────────────────────────┐
   │ MCOA: L_tissue = Σ wᵢ fᵢ(Dᵢ) — Dᵢ candidates include    │
...
| Hub | Why |
|-----|-----|
| `LongevityCommon/CONCEPT.md` (this regenerate) | Single source of cross-cutting status; bug here propagates to all subprojects + grant docs. |
...
| Old `~/Desktop/LongevityCommon/CONCEPT.md` (pre-v5) | Replaced today | Already moved to archive |
...
When article (`~/Desktop/LongevityCommon.md`) changes:
...
4. Cross-check subproject CONCEPTs (Ze/BioSense/FCLC/CDATA/MCOA) for conflicts; update where needed
...
| MCOA | (none — landing only) | static `/var/www/mcoa-landing/` | mcoa.longevity.ge |
