...
Path: `/home/oem/Desktop/LC/MCAOA`  Date: 2026-05-08

## Size & file counts
```
832K	/home/oem/Desktop/LC/MCAOA
```
...
**MCAOA** (Multi-Counter Architecture of Organismal Aging) — мета-теоретическая mother-project всего LC-стека. Формализует организменное старение как взвешенную сумму параллельных damage-counter'ов с tissue-specific весами `w_i`.

**Path:** `/home/oem/Desktop/LC/MCAOA/`
**Repo:** часть монорепозитория `djabbat/LC` (private) / `djabbat/LC-public` (public, без core .md и Patients/).

---

## Source of truth

**`MCAOA/CONCEPT.md`** — авторитетный документ. Любые конфликты между этим CLAUDE.md и CONCEPT.md решаются в пользу CONCEPT.md. Канонический референс — `~/Documents/MCOA_NatureAging_submission/01_MCOA_Perspective_manuscript.md`.

См. также корневой `~/Desktop/LC/CLAUDE.md` (parent rules).
...
Все 5 counter'ов — active-development subprojects (per user decision 2026-05-07: «отвечают за лимит Хейфлика индивидуально и крест-накрест, входят в MCAOA как треки»).
...
- **Backend:** Rust (Cargo workspace `MCAOA/backend/`), crates под `MCAOA/crates/`
- **Frontend:** React/TypeScript (`MCAOA/frontend/`)
...
2. Любое изменение counter-equation (Dᵢ kinetics) требует обновления соответствующего subproject CONCEPT.md и MCAOA/MAP.md.
...
`cargo test` в `MCAOA/` workspace — должно проходить полностью перед коммитом в `main`.
...
*   **Фальсифицируемость — первоклассная цель:** Каждое утверждение, выведенное из MCAOA, должно сопровождаться чётким экспериментальным тестом, который может его опровергнуть (Аксиома M4).
...
## Связь с другими проектами LC

*   **[THEORY.md](THEORY.md):** Полное формальное изложение аксиом, уравнений и предсказаний MCAOA.
*   **[EVIDENCE.md](EVIDENCE.md):** Проверенные литературные источники (PMID/DOI), внутренние данные и опровергающие свидетельства, поддерживающие или оспаривающие MCAOA.
*   **[OPEN_PROBLEMS.md](OPEN_PROBLEMS.md):** Неразрешённые научные проблемы, приоритеты и конкретные фальсифицируемые тесты для MCAOA.
...
*   **[AGENTS.md](AGENTS.md):** Инструкции и жёсткие правила для LLM-агентов, работающих с кодом и документацией MCAOA.
...
All Γ entries must be measured, not fitted. ~~MCAOA Test 2~~ [отозвано — see CORRECTIONS §1.3] (§6.2 Perspective) is the canonical measurement protocol.
...
**Version:** 1.0
**Date:** 2026-04-21
**Next revision trigger:** Nature Aging editorial decision OR completion of MCAOA Test 4 simulation.


---

## Роль MCAOA в EIC Pathfinder Part B v3 (Variant B, submission 2026-05-12)
...
1. Публикация MCAOA specification paper (открытый стандарт)
...
*   **To be calibrated:** Требует калибровки на данных в рамках MCAOA.
...
```md
# Открытые проблемы и фальсифицируемые тесты для MCAOA
...
**Приоритет:** **Высокий** (P0). Блокирует полноценную экспериментальную проверку MCAOA.
...
**Возможные исходы:**
1.  **✅ Сильная корреляция (R² > 0.7):** Подтверждает возможность *априорного* предсказания весов. MCAOA проходит этот тест.
...
4.  **🔀 Противоречивый результат:** Разные методы предсказания дают сильно различающиеся `w_i_pred`. Указывает на фундаментальную неопределённость в выборе *априорных* параметров, что ослабляет всю конструкцию MCAOA.
...
**Возможные исходы:**
1.  **✅ Замедление всех счётчиков и фенотипов:** Сильно поддерживает upstream-роль CP. MCAOA с CP #1 подтверждён.
...
4.  **🔀 Тканеспецифичный эффект:** Замедление наблюдается только в высокопролиферативном эпителии кишечника, но не в печени. Подтверждает контекстно-зависимую роль CP, что согласуется с MCAOA, но усложняет общую теорию.

## Проблема 3: Измерение матрицы связей Γ и проверка гипотезы `γ_i = 0`

**Описание:** По умолчанию в MCAOA принимается `γ_i = 0` (гипотеза независимости счётчиков). Ненулевые связи должны быть доказаны. Не существует стандартизированного in vivo протокола для прямого измерения элементов `Γ_{ij}`.

**Приоритет:** **Средний** (P2). Критично для предсказания синергии интервенций.

### Фальсифицируемый тест 3A: Протокол MCAOA Test 2 (in vitro калибровка)
...
4.  **🔀 Нелинейная зависимость:** Эффект наблюдается только после определённого порога повреждения митохондрий. Потребует модификации линейной модели связи в MCAOA на пороговую или сигмоидальную.
...
**Гипотеза:** В культуре фибробластов, где доминирует теломерный счётчик, вычисленная по MCAOA нагрузка `L_tissue` будет монотонно расти с пассажами и достигнет постоянного значения `L_critical` в момент вхождения в сенесценцию (лимит Хейфлика).
