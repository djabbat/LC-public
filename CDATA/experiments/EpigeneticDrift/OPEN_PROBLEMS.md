...
> All proposed tests (A–D) will be pre-registered with detailed statistical analysis plans (OSF ID: osf.io/TBD (placeholder, to be updated upon pre-registration)). Sample size calculations are provided in PARAMETERS.md.
 ...
 * **Исход A (подтверждает первичность ABL-2):** Изменение ABL-2 ведёт к пропорциональному, однонаправленному сдвигу всех эпигенетических часов в ткани. **Количественное правило:** |ΔD₄| > 0.15, p < 0.01, power ≥ 0.80.
 * **Исход B (ослабляет):** Изменение ABL-2 не влияет на часы, но влияет на другие фенотипы старения. **Количественное правило:** |ΔD₄| ≤ 0.15, p ≥ 0.05.
 * **Исход C (опровергает простую модель):** Изменение ABL-2 влияет на одни часы (например, Horvath), но не на другие (например, DunedinPACE), указывая на сложную, нелинейную связь. **Количественное правило:** Коэффициент вариации эффекта между часами > 0.5.
 * **Исход D (неразрешимо):** Технические шумы или адаптационные механизмы маскируют эффект. **Количественное правило:** Отношение сигнал/шум < 2.
...
|------|-------------------|--------------|------------|
| ABL‑2 confound invalidates D₄ as independent counter | 3 | 5 | Parallel experimental test of ABL‑2 modulation (OPEN_PROBLEMS §1) |
| Inability to separate β₄ and α₄ in vivo | 4 | 3 | CRISPR‑barcoding lineage tracing (OPEN_PROBLEMS §2) |
| No universal D₄ exists → vector model needed | 3 | 3 | Pre‑planned factor analysis on multi‑dimensional epigenomic data |
| Coupling coefficients γ₄ⱼ are not measurable | 2 | 4 | Use conservative default γ=0; alternative: measure via pairwise perturbations |
| Insufficient tissue diversity for clock training | 2 | 2 | Include datasets with ≥10 tissues from GTEx, ENCODE, etc. |

## Consortium / partners

No external consortium or collaboration plan has been established yet. Potential partners for replication and validation include:

- **Epigenetic clock consortia** (e.g., the Aging Clock Consortium, if applicable)
- **Stem cell aging labs** (e.g., groups working on HSC aging, such as Bogeska et al. 2022, PMID: 35858618)
- **Clinical trial networks** (e.g., DO-HEALTH, Bischoff-Ferrari et al. 2025, PMID: 39900648)

Formal agreements and data-sharing plans are TBD.

## Reproducibility & open science

**Code repository:** Not yet available. A public repository (e.g., GitHub) will be created upon manuscript acceptance, containing analysis scripts for all clock calculations and statistical tests.

**Data deposit plan:** Raw and processed DNA methylation data will be deposited in a public repository (e.g., GEO, Zenodo, or OSF) upon publication.

**Pre-registration:** The study plan will be pre-registered at OSF (ID: osf.io/TBD, placeholder; date TBD).

**Materials transparency:** A full list of software dependencies (e.g., R packages, Python libraries) will be provided in a `requirements.txt` or `environment.yml` file. Protocols for cell culture and methylation assays will be shared via protocols.io (link TBD).

## Pre-registration plan

The pre-registration for all proposed tests (A–D) will be submitted to the Open Science Framework prior to data collection. The OSF ID placeholder is osf.io/TBD (to be updated upon pre-registration). The planned pre-registration date is 2026-09-01. The pre-registration will include: (1) primary and secondary hypotheses for each test, (2) statistical analysis plans with explicit stopping rules, (3) sample size justifications based on power analyses in PARAMETERS.md, and (4) a data management plan specifying that all raw and processed data will be deposited in a public repository (Zenodo or OSF) upon publication.
